...
export default function Pricing() {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto px-6 py-12 gap-8 min-h-screen">
      {isLoggedIn && (
        <aside className="w-full lg:w-64 flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible shrink-0 pt-4 lg:pt-12 border-b lg:border-b-0 lg:border-r border-border pb-4 lg:pb-0 lg:pr-8 no-scrollbar">
          <div className="px-4 text-xs font-semibold text-muted uppercase tracking-wider mb-2">Menu</div>
          <button onClick={() => router.push('/dashboard?view=documents')} className="text-left px-4 py-3 rounded-xl text-muted hover:bg-muted/5 hover:text-foreground transition-colors">
            Extract Invoices
          </button>
          <button onClick={() => router.push('/dashboard?view=documents')} className="text-left px-4 py-3 rounded-xl text-muted hover:bg-muted/5 hover:text-foreground transition-colors">
            Invoice History
          ...
          <div className="px-4 text-xs font-semibold text-muted uppercase tracking-wider mt-6 mb-2">Settings</div>
          <button onClick={() => router.push('/dashboard?view=profile')} className="text-left px-4 py-3 rounded-xl text-muted hover:bg-muted/5 hover:text-foreground transition-colors">
            Update Profile
          </button>
          <button onClick={() => router.push('/dashboard?view=developer')} className="text-left px-4 py-3 rounded-xl text-muted hover:bg-muted/5 hover:text-foreground transition-colors">
            Developer Portal
          </button>
          <button className="text-left px-4 py-3 rounded-xl bg-blue-600/10 text-blue-400 font-medium border border-blue-500/20">
            Subscription Plan
          </button>
          <button onClick={() => { localStorage.removeItem('access_token'); router.push('/login'); }} className="text-left px-4 py-3 rounded-xl text-red-500 hover:bg-red-500/10 transition-colors mt-auto">
            Logout
          </button>
        </aside>
      )}
      ...
      <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-bottom-8 duration-700 pt-12">
        <div className="text-center mb-12 flex flex-col items-center">
          <h1 className="text-5xl font-black text-foreground mb-6 tracking-tighter uppercase italic">Simple, transparent <span className="text-blue-600">pricing</span></h1>

          {/* Billing Toggle */}
          <div className="flex items-center justify-center gap-4 bg-muted/5 p-2 rounded-2xl border border-border mb-8 backdrop-blur-md">
            <span className={`text-sm font-medium transition-colors ${billingCycle === 'month' ? 'text-foreground' : 'text-muted'}`}>Monthly Billing</span>
            <button
            ...
            <span className={`text-sm font-medium transition-colors ${billingCycle === 'year' ? 'text-foreground' : 'text-muted'}`}>
          ...
          <p className="text-muted text-lg max-w-2xl">Choose the plan that's right for your business. Switch or cancel any time.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 max-w-[95rem] mx-auto w-full px-4 mb-20">
          {Array.isArray(plans) && plans
            ...
            .map((plan, i) => (
            <div key={plan.id} className={`p-6 rounded-3xl border flex flex-col transition-all relative group h-full ${plan.name === 'Pro' ? 'bg-blue-600/5 border-blue-500/30 shadow-[0_0_50px_rgba(37,99,235,0.1)] hover:bg-blue-600/10 z-10' : 'bg-card border-border hover:bg-muted/5'}`}>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-black text-foreground mb-1 tracking-tight uppercase">{plan.name}</h3>
              ...
              <div className="mb-6">
                <div className="flex items-baseline gap-1">
                  ...
                  {plan.name !== 'Enterprise' && (
                    <span className="text-muted font-medium text-[10px]">/{plan.billingCycle === 'year' ? 'yr' : 'mo'}</span>
                  )}
                </div>
              ...
              <ul className="flex flex-col gap-3 text-muted mb-8 flex-1 border-t border-border pt-6">
                <li className="flex items-center gap-2 text-[13px]">
                  <span className="text-blue-600 font-bold">✓</span>
                  <span className="font-black text-foreground">{plan.name === 'Enterprise' ? 'Unlimited' : plan.quotaPages.toLocaleString()}</span> Pages {plan.name === 'Enterprise' ? '' : (plan.billingCycle === 'year' ? '/yr' : '/mo')}
                </li>
                {plan.name === 'Free' ? (
                  <li className="flex items-center gap-2 text-[13px] text-muted">
                    <span className="text-muted">×</span> Basic Extraction Only
                  </li>
                ) : (
                  <li className="flex items-center gap-2 text-[13px]">
                ...
                {plan.name === 'Starter' || plan.name === 'Free' ? (
                   <li className="flex items-center gap-2 text-[13px] text-muted">
                    <span className="text-muted">×</span> Batch Processing
                  </li>
                ) : (
                ...
                {plan.name === 'Business' || plan.name === 'Enterprise' ? (
                  <li className="flex items-center gap-2 text-[13px] text-indigo-300 font-bold">
                   ...
                   <li className="flex items-center gap-2 text-[13px] text-muted">
                    <span className="text-muted">×</span> API Access
                  </li>
                )}
              ...
              <button 
                ...
                className={`w-full py-3 rounded-xl text-white text-sm font-bold tracking-tight transition-all active:scale-95 mt-auto ${plan.name === 'Pro' ? 'bg-blue-600 hover:bg-blue-500 shadow-[0_0_20px_rgba(37,99,235,0.4)]' : 'bg-foreground dark:bg-muted/10 border border-border hover:bg-foreground/90'}`}>
