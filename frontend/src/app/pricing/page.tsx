...
export default function Pricing() {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full max-w-7xl mx-auto px-6 py-12 gap-8 min-h-screen">
      {isLoggedIn && (
        <aside className="w-full lg:w-64 flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible shrink-0 pt-4 lg:pt-12 border-b lg:border-b-0 lg:border-r border-white/5 pb-4 lg:pb-0 lg:pr-8 no-scrollbar">
          ...
          <button onClick={() => { localStorage.removeItem('access_token'); router.push('/login'); }} className="text-left px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-colors mt-auto">
            Logout
          </button>
        </aside>
      )}
